...
<p align="left">
  ...
  <img src="https://img.shields.io/badge/CMake-3.12%2B-064F8C.svg" alt="CMake Version" />
...
- 🛠️ **CMake Build System:** Cross-platform build management for Linux, macOS, and Windows.
- 📅 **Events & Deadlines:** Assign due dates/times (`due:YYYY-MM-DD` or `due:HH:MM`) and view them via `task events`.
...
### 2. Direct Compilation with GCC / Clang
...
#### Linux / macOS
sudo mv build/task /usr/local/bin/

#### Windows (PowerShell)
1. Move task.exe to a permanent directory (e.g., C:\Tools\TaskCLI\).
...
| `task add "Meeting" due:14:30 @work` | Add a scheduled event/task with a due time/date |
...
| `task edit <ID> "Title" due:18:00` | Edit title, priority, tags, or due date |
